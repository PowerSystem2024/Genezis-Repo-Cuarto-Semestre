...
document.addEventListener("DOMContentLoaded", () => { //(9)Espera a que el DOM cargue antes de ejecutar el script
class Avatar {
...
function init(){
  ...
  // 2. Limpiamos el contenedor antes de poblarlo
  cont.innerHTML = '';

  // 3. Recorremos el array 'avatares' con forEach 
  avatares.forEach(avatar => {
    
    // 4. Creamos el string HTML para CADA avatar.
    const tarjetaAvatarHTML = `
      <button class="personaje" data-nombre="${avatar.nombre}">
    ...
    // 5. Añadimos el HTML de esta tarjeta al contenedor
    // ¡Este es el método 'innerHTML +='
    cont.innerHTML += tarjetaAvatarHTML;
  });
