...
// MODIFICADO: Añadido 'next' para el manejo de errores
export const signin = async (req, res, next) => {
    const {email, password} = req.body;

    // MODIFICADO: Añadido bloque try...catch
    try {
        ...
        const token = await createAccessToken({id: result.rows[0].id});
            ...
            res.cookie("token", token, {
                httpOnly: true, // MODIFICADO: Descomentado por seguridad
                secure : true,
                sameSite: "none",
        ...
        // MODIFICADO: Capturamos cualquier error de DB y lo pasamos al manejador
        next(error);
    }
}
